...
impl PerfTracker {
    fn render_chart(&self, ui: &imgui::Ui, origin: [f32; 2], bounds: [f32; 2], value_max: f32) {
        ...
        draw_list.add_rect(origin, [ origin[0] + bounds[0], origin[1] + bounds[1] ], ui.style_color(StyleColor::FrameBg))
            .filled(true)
            .build();
        ...
        let bar_space = 0.0;
        let bar_width = (bounds[0] - (self.history_length - 1) as f32 * bar_space) / self.history_length as f32;
        ...
        for (marker_index, marker) in self.markers.iter().enumerate() {
            for bar_index in 0..self.history_length {
                ...
                draw_list.add_rect(
                    [ origin[0] + bar_x_offset, origin[1] + bounds[1] - bar_y_offset - bar_height ], 
                    [ origin[0] + bar_x_offset + bar_width, origin[1] + bounds[1] - bar_y_offset ], 
    ...
    fn generate_marker_labels(&self, ui: &imgui::Ui, max_width: f32) -> Vec<MarkerLabel> {
        ...
        for (marker_index, marker_name) in self.marker_names.iter().enumerate() {
            ...
            let avg = marker.iter().cloned().reduce(|a, b| a + b).unwrap_or(0.0) / self.history_length as f32;
            let var = marker.iter().cloned()
                .map(|v| (v - avg) * (v - avg))
                .reduce(|a, b| a + b)
                .unwrap_or(0.0) / self.history_length as f32;

            let text = format!("{} (max: {:.2}ms, avg: {:.2}ms, std: {:.2}ms)", 
                marker_name, 
    ...
    pub fn render(&self, ui: &imgui::Ui, bounds: [f32; 2]) {
        ...
        let label_height = marker_labels.iter().map(|label| label.y + label.height).reduce(f32::max).unwrap_or(50.0);

        let chart_bounds = [ bounds[0] - 75.0, bounds[1] - label_height - 5.0 ];
        let value_max = self.marker_total.iter().cloned().reduce(f32::max).unwrap_or(1.0).ceil();
        ...
        /* render bar labels */
        {
            ...
            for (label_index, label) in marker_labels.into_iter().enumerate() {
                draw_list.add_text(
                    [ origin[0] + label.x, origin[1] + chart_bounds[1] + 5.0 + label.y ], 
