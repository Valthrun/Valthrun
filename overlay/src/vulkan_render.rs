...
use crate::{
    error::Result,
    vulkan_driver::get_vulkan_entry, OverlayError,
};
...
fn create_vulkan_instance(
    ...
    log::debug!("Detected vulkan version {}.{}.{}", vk::api_version_major(instance_version), vk::api_version_minor(instance_version),  vk::api_version_patch(instance_version));
    ...
    let instance = unsafe { entry.create_instance(&instance_create_info, None).map_err(OverlayError::VulkanInstanceCreationFailed)? };
