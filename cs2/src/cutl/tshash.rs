...
use cs2_schema::{define_schema, MemoryHandle, SchemaValue};

use crate::{Ptr, CS2Handle};
...
impl<K: SchemaValue, V: SchemaValue> HashBucketData<K, V> {
    fn value(&self) -> anyhow::Result<V> {
        SchemaValue::from_memory(&self.memory, self.offset + 0x00)
    }

    fn key(&self) -> anyhow::Result<K> {
        SchemaValue::from_memory(&self.memory, self.offset + V::value_size() as u64 + 0x08)
    }
}

impl<K: SchemaValue, V: SchemaValue> SchemaValue for HashBucketData<K, V> {
    fn value_size() -> usize {
        K::value_size() + V::value_size() + 0x08
    }
...
impl<K: SchemaValue, V: SchemaValue> HashUnallocatedData<K, V> {
    ...
    pub fn bucket_entry(&self, index: usize) -> anyhow::Result<HashBucketData<K, V>> {
        SchemaValue::from_memory(&self.memory, self.offset + 0x20 + (HashBucketData::<K, V>::value_size() * index) as u64)
    }
}
...
impl<K: SchemaValue, V: SchemaValue> SchemaValue for HashUnallocatedData<K, V> {
    fn value_size() -> usize {
        ...
        0x20 + HashBucketData::<K, V>::value_size() * 256
    }
...
impl<K: SchemaValue, V: SchemaValue, const N: usize> CUtlTSHash<K, V, N> {
    ...
    pub fn bucket(&self, index: usize) -> anyhow::Result<HashBucket<K, V>> {
        SchemaValue::from_memory(&self.memory, self.offset + (CUtlMemoryPool::value_size() + index * HashBucket::<K, V>::value_size()) as u64)
    }

    pub fn read_values(&self, cs2: &CS2Handle) -> anyhow::Result<Vec<V>> {
        ...
        while current_data.address() > 0 && num_entries_remaining > 0 {
            let data_array = current_data.read_schema(cs2)?;
...
impl<K: SchemaValue, V: SchemaValue, const N: usize> SchemaValue for CUtlTSHash<K, V, N> {
    fn value_size() -> usize {
        CUtlMemoryPool::value_size() + N * HashBucket::<K, V>::value_size()
    }
